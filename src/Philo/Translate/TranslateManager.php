...
class TranslateManager
{
    ...
    {
        if (in_array($abbreviation, $this->languages)) {
            return;
        }
    ...
    {
        if ($loaded = array_get($this->loaded, $this->language . "." . $group)) {
            return $loaded;
        }
    ...
    protected function getGroup($name)
    {
        ...
        return (Lang::has($name)) ? Lang::get($name) : [];
    }
    ...
    public function removeLine($group, $line)
    {
        foreach ($this->languages as $language) {
            ...
            $this->writeToFile($group, array_get($this->loaded, "$language.$group", []));
        }
    ...
    {
        return App::make('Finder')
                  ->files()
                  ->name('*.php')
                  ->in(base_path())
                  ->exclude($this->getIgnoredFolders())
                  ->contains("$group.$line")
                  ->count();
    }
    ...
    {
        $files = [];
        $results = App::make('Finder')
                      ->files()
                      ->name('*.php')
                      ->in($this->getLanguagePath());
